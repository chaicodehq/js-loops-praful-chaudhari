...
export function upiRetry(outcomes) {
    ...
    do {
        attempt++;
        const trial = outcomes[attempt];

        if (trial === "success") {
            isSuccess = true;
            break;
        } else {
            waitTime = waitTime * 2 + 1;
        }
    } while (attempt < outcomes.length && attempt < 4);
    ...
    return { attempts: attempt, totalWaitTime: waitTime, success: isSuccess };
}
